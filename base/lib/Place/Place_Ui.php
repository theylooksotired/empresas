...
class Place_Ui extends Ui
{
    ...
    public function renderEmail()
    {
        ...
        $tags = new ListObjects('Tag', [
            'table' => 'Tag, PlaceEditTag',
            'object' => 'Tag',
            'fields' => 'DISTINCT ' . Db::prefixTable('Tag') . '.*',
            'where' => '' . Db::prefixTable('PlaceEditTag') . '.idTag=' . Db::prefixTable('Tag') . '.idTag AND ' . Db::prefixTable('PlaceEditTag') . '.idPlaceEdit="' . $this->object->id() . '"',
        ]);
        return '
			...
			<p style="margin: 0 0 5px; padding: 0;">
				...
				<span>' . substr($tags->showList(['function' => 'Simple']), 0, -2) . '</span>
			</p>
			' : '') . '
			<div style="width: 120px; height: 10px; background: #efefef; margin: 30px 0;"> </div>';
    }
